...
export function createNotificationString(notification: KhanAcademyNotification): string {
  ...
  switch (notification.__typename) {
    ...
    case 'ProgramFeedbackNotification':
      ...
      } on ${cleanse(notification.translatedScratchpadTitle)}</a><span class="notification-date">${timeSince(
        new Date(date),
      )} ago</span></div><div class="notification-content">${parseMarkdown(
        notification.content,
    ...
    case 'ModeratorNotification':
      return `<li class="notification ${
        ...
        new Date(date),
      )}</span></div><div class="notification-content">${parseMarkdown(notification.text)}</div></li>`;
    default:
      ...
      return `<li class="notification ${
        ...
        new Date(date),
      )} ago</span></div><div class="notification-content">${JSON.stringify(notification)}</div></li>`;
  }
}
...
async function sendMessage(event: MouseEvent): Promise<void> {
  ...
  if (url === undefined || typename === undefined || feedbacktype === undefined) {
    handleError(`at least one of url: ${url}, typename: ${typename}, feedbacktype: ${feedbacktype} is undefined`);
    return;
  }
